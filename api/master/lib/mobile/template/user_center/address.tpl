...
<div class="mui-content ziima">
	<header class="mui-bar mui-bar-nav" style="background-color: white;">
		<a class="mui-action-back mui-icon mui-icon-left-nav mui-pull-left" style="color: #666666;" href="{:$ReUrl:}"></a>
		<h1 class="mui-title" style="font-size: 16px;">地址管理</h1>
	</header>
	<div class="mui-content">
		<ul class="mui-table-view" style="margin-top: 10;width: 94%;margin-left: 3%;"></ul>
	</div>
...
<script type="text/javascript">
    ...
    $(".mui-but").on("tap", function() {
		window.location.href = "/mobile.php?call=user_center.address_add";
	});
		...
		}, function (t, q, r) 
		{
			...
			$(".address_edit").on("tap", function() {
				var aid=$(this).attr('data-id');
				 ...
				 mui.openWindow({
					 "url": '/mobile.php?call=user_center.address_edit&share_id='+"{:$share_id:}&"+"id="+aid,
					 "styles": {
						 "popGesture": "none"
					 }
				 });
			 ...
			 $(".sc").on("tap", function(){
				...
				YueMi.API.invoke('profile','address_del',addressnew,function(target, request, response){
					 ...
					 mui.toast('删除地址成功！');
					 //location.reload();
					 window.location.href = '/mobile.php?call=user_center.address&share_id='+"{:$share_id:}&"+"res_id="+defaultid;
	...
	$('.mui-table-view').on("click", "li .mui-table-view-cell1 .check1", function(ev) {
		...
		YueMi.API.invoke('profile', 'address_default', {
			...
			window.location.href = "/mobile.php?call=user_center.address_set_default&id="+defaultid;
		}, function(target, request, response) {
			console.log(11);
			return;
		});
