...
<div class="mui-content ziima">
    ...
    <div class="mui-content">
    	<ul class="mui-table-view" style="background:red;">
		    ...
		    <li class="mui-table-view-cell addresssected">
		        ...
		        <a href="/mobile.php?call=user_center.address&share_id={:$share_id:}" class="mui-navigate-right" style="font-size: 12px;color: white;white-space:nowrap;">请添加设置默认收货地址</a>
			...
			<li class="mui-table-view-cell addresssected">
		        ...
		        <a  href="/mobile.php?call=user_center.address&share_id={:$share_id:}" class="mui-navigate-right" style="font-size: 12px;color: white;white-space:nowrap;">{:$data.province:}{:$data.city:}{:$data.country:}</a>
	...
	<div class="mui-but" style="background-color:#F2493D;height: 40px;line-height: 17px;" id="tj">去支付<span id="pay_money">{:$result.price_sale | number.currency:}</span></div>
...
function change_money(){
	...
	var money = Number(document.getElementById('MyMoney').innerHTML);
	var need_money = Number(document.getElementById('PriceTotal').innerHTML);
	var pay = 0;
	if (status){
		if (money > need_money){
			pay = 0;
...
{   
	...
	YueMi.API.invoke('order', 'fast_order', {
			...
			if (response.status === 1) {
				...
				// 已支付
				window.location.href = '/mobile.php?call=user_center.success&order_id='+response.order_id;
			} else {
				window.location.href = '/mobile.php?call=user_center.order&order_id='+response.order_id;
	...
	function onBridgeReady()
	{
		WeixinJSBridge.invoke(
			 'getBrandWCPayRequest', {
				 "appId":"{:#WXAPP_APPID:}", // 公众号名称，由商户传入     
				 "timeStamp":"{:#Z_NOW:}", // 时间戳，自1970年以来的秒数
				 "nonceStr":"{:$nonceStr:}", // 随机串     
				 "package":"prepay_id=wx212109466566714133f3318a3221312279",     
