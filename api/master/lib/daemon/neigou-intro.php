...
$MAP = [];
echo "处理外部SPU\n";
$list = $mysql->grid("SELECT `id`,`bn` FROM `yuemi_sale`.`ext_spu` WHERE `lo_status` < 2 ORDER BY `id` ASC");
foreach ($list as $spu) {
	$id = $spu['id'];
	$bn = $spu['bn'];
	echo " $bn...";
...
echo "处理外部SKU\n";
$list = $mysql->grid("SELECT `id`,`bn` FROM `yuemi_sale`.`ext_sku` WHERE `lo_status` < 2  ORDER BY `id` ASC");
foreach ($list as $spu) {
	$id = $spu['id'];
	$bn = $spu['bn'];
	echo " $bn...";
	...
	$mysql->execute("UPDATE `yuemi_sale`.`ext_sku` SET `intro` = '%s',`lo_status` = 2,`lo_time` = UNIX_TIMESTAMP() WHERE `id` = $id", $mysql->encode($txt));
}