...
/*
 微信登陆
*/
DROP PROCEDURE IF EXISTS `login_wechat` |||
CREATE PROCEDURE `login_wechat` (
...
Main : BEGIN
	...
	START TRANSACTION;
		...
		IF SYSEmpty = 1 THEN
			...
			END IF;
			INSERT INTO `user_wechat` (`user_id`,`invitor_id`,`app_open_id`,`union_id`,`name`,`avatar`,`gender`,`create_time`,`create_from`,`tag_seed`,`tag_param`)
			VALUES (0,InvitorId,WxOpenId,WxUnionId,WxName,WxAvatar,WxGender,UNIX_TIMESTAMP(),ClientIp,InvitorSeed,InvitorParam);
			IF SYSError = 1 THEN
				ROLLBACK;
