...
DROP PROCEDURE IF EXISTS `profit_self_income` |||
CREATE PROCEDURE `profit_self_income` (
...
) LANGUAGE SQL NOT DETERMINISTIC SQL  SECURITY INVOKER CONTAINS SQL READS SQL DATA MODIFIES SQL DATA COMMENT '自买佣金收入'
Main : BEGIN
	...
	-- 开启事务
	START TRANSACTION;
		SELECT `profit_self` INTO Profit_Old FROM `user_finance` WHERE `user_id` = UserId FOR UPDATE;
		IF SYSEmpty = 1 THEN
			ROLLBACK;
		...
		UPDATE `user_finance` SET `profit_self` = Profit_New WHERE `user_id` = UserId;
		IF SYSError = 1 THEN
			ROLLBACK;
			SET ReturnValue = 'E_FINANCE';
		...
		VALUES (UserId,'SELF',SrcType,SrcId,Profit_Old,Profit_Dlt,Profit_New,Message,UNIX_TIMESTAMP(NOW()),ClientIp);
		IF SYSError = 1 THEN
			ROLLBACK;
...
DROP PROCEDURE IF EXISTS `profit_self_expend` |||
CREATE PROCEDURE `profit_self_expend` (
	IN UserId INT UNSIGNED,		-- 用户ID
	IN Profit_Dlt NUMERIC(16,4),		-- 增加额度
	IN SrcType VARCHAR(16),		-- 原因
	IN SrcId VARCHAR(24),		-- 关联ID
	...
	OUT ReturnValue	VARCHAR(32),
	OUT ReturnMessage VARCHAR(1024)
) LANGUAGE SQL NOT DETERMINISTIC SQL  SECURITY INVOKER CONTAINS SQL READS SQL DATA MODIFIES SQL DATA COMMENT '自买佣金支出'
Main : BEGIN
	DECLARE SYSError INT DEFAULT 0;
	DECLARE SYSEmpty INT DEFAULT 0;
