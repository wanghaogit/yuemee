...
/*
	后台卡位总监
*/
DROP PROCEDURE IF EXISTS `make_money_director_ex` |||
CREATE PROCEDURE `make_money_director_ex` (
	...
	IN IsGiveCheifCard TINYINT UNSIGNED,	-- 是否给30张总监卡
...
Main : BEGIN
	...
	START TRANSACTION;
		...
		-- ----------------------------------------------------------------TODO吧总监下了
		SELECT * FROM `yuemi_main`.`cheif` WHERE `user_id` = UserId;
		IF SYSEmpty = 1 THEN 
			SET SYSEmpty = 0;
		...
		-- 总经理
		SET ReturnMessage = CONCAT(ReturnMessage,'->','插经理');
		SET InviteCode = RAND_STRING(8);
		INSERT INTO `director` (`user_id`,`status`,`create_time`,`update_time`,`expire_time`)
		VALUES (UserId,3,UNIX_TIMESTAMP(),UNIX_TIMESTAMP(),UNIX_TIMESTAMP() + 31536000);
		IF SYSError = 1 THEN
			SET ReturnValue = 'E_DATABASE';
			SET ReturnMessage = CONCAT(ReturnMessage,'->','经理错');
			ROLLBACK;
			LEAVE Main;
		END IF;
		...
		INSERT INTO `director_buff` (`user_id`,`type`,`order_id`,`pay_channel`,`pay_status`,`pay_time`,`money`,`start_time`,`expire_time`,`create_time`,`create_from`)
		VALUES (UserId,3,OrderId,2,2,UNIX_TIMESTAMP(),120000.00,UNIX_TIMESTAMP(),UNIX_TIMESTAMP() + 31536000,UNIX_TIMESTAMP(),ClientIp);
		IF SYSError = 1 THEN
			SET ReturnValue = 'E_DATABASE';
		...
		UPDATE `user` SET `level_d` = 3 WHERE `id` = UserId;

		-- 赠送30张总监卡
		IF IsGiveCheifCard > 0 THEN
			...
			END WHILE;
		ELSE
			SET ReturnMessage = CONCAT(ReturnMessage,'->','不给卡');
		END IF;
