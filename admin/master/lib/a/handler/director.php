 ...
 * 经理管理接口
 */
class director_handler extends ApiHandler {
	...
	public function helicopter(\Ziima\MVC\REST\Request $request) {
		if (!empty($request->body->order_id) && !preg_match('/^KD\d{6}[A-Z0-9]+$/', $request->body->order_id)) {
		...
		$ret = \yuemi_main\ProcedureInvoker::Instance()->make_money_director_ex(
				$request->body->mobile,
		...
		try {
			...
			if ($request->body->give_card) {
				$sms->send($request->body->mobile, 'SMS_134318037', ['name' => $request->body->name]);
			} else {
				$sms->send($request->body->mobile, 'SMS_134328150', ['name' => $request->body->name]);
			}
		} catch (\Exception $e) {
