...
<table border="0" cellspacing="0" cellpadding="0" class="Grid">
	<caption>
		<a href="/index.php?call=cheif.create" class="button" style="float:left;">录入总监</a>
		<a href="/index.php?call=user.vip_card" class="button" style="float:right;">VIP卡</a>
		总监列表
	</caption>
	<tr>
		<td>
	...
	<tr>
		...
		<td>{:$v.user_id:}</td>
		<td><a href="/index.php?call=user.vip_card&uid={:$v.user_id:}">{:$v.name | string.key_highlight $_PARAMS.n:}</a></td>
		<td>{:$v.cname:}</td>
		<td align="right">{:$v.mobile | string.key_highlight $_PARAMS.m:}</td>
		<td align="center">{:$v.drcname:}</td>
		<td>{:$v.status | boolean.iconic:}</td>
		<td>{:$v.expire_time | number.datetime:}</td>
		<td><a href="/index.php?call=cheif.cheifinv_pic&uid={:$v.user_id:}">查看族谱</a>
			|
