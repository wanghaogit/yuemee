...
<table border="0" cellspacing="0" cellpadding="0" class="Grid">
	<tr>
		...
		<td colspan="13">
			<form action="/index.php?call=sku.sku_b" method="GET" name="form1">
	...
	<tr>
		...
		<td style="font-size:14px">
			...
			<a href="/index.php?call=sku.sku_detail&id={:$SKU.id:}">{:$SKU.title| string.key_highlight $_PARAMS.q:}</a>
		</td>
		<td>{:$SKU.subtitle:}</td>
		<td></td>
		<td>
		...
		<td>
			<ul>
				...
				<li>无邀请码价格：<span style="float:right;">{:$SKU.price_sale | number.currency:}</span></li>
		...
		<td >
			{:if $SKU.status == 0:}待审
			{:elseif $SKU.status == 1:}<span style="color:red;">驳回</span>
			{:elseif  $SKU.status == 2:}上架
			{:elseif  $SKU.status == 3:}下架
			{:elseif  $SKU.status == 4:}删除
			{:else:}
			{:/if:}
		</td>
		<td class="operator">
			...
			<a href="/index.php?call=sku.update_sku&id={:$SKU.id:}&p={:$_PARAMS.p:}&ty=b">编辑详情</a>
