...
<form name="form1" action="/index.php?call=runer.source_update&id={:$id:}" method="post" id="form1">
	<ul class="Form">
		<li>
			<label>数据源名称：</label>
		...
		<li>
			<label>数据源类型：</label>
			<select name="style" style="width:100px;background-color:#fff;height:27px" id="style"  onchange="javascript:select_menu(parseInt(this.options[this.selectedIndex].value));">
				<option value="0" {:if $Result.style == 0:}selected="selected"{:/if:}>SQL</option>
				<option value="1" {:if $Result.style == 1:}selected="selected"{:/if:}>PHP</option>
				<option value="2" {:if $Result.style == 2:}selected="selected"{:/if:}>选定商品</option>
				<option value="3" {:if $Result.style == 3:}selected="selected"{:/if:}>商品筛选</option>
				<option value="4" {:if $Result.style == 4:}selected="selected"{:/if:}>选定专题</option>
			</select>
			<label>数据源格式：</label>
			<select name="type" style="width:100px;background-color:#fff;height:27px">
				<option value="0" {:if $Result.type == 0:}selected="selected"{:/if:}>自定义</option>
				<option value="1" {:if $Result.type == 1:}selected="selected"{:/if:}>单品</option>
				<option value="2" {:if $Result.type == 2:}selected="selected"{:/if:}>多品</option>
			</select>
		...
		<li id="chooseli" style="display:none;">
			<select id='isno'>
				<option value='one'>单品</option>
				<option value='special'>专题</option>
			</select>
			<input type='button' value='选择商品（SKU）' onclick="choosegood();"/>
			<input type='button' value='生成数据源' onclick="pushit();"/>
		</li>
		<li>
			<label>驱动代码</label>
		</li>
		<li>
			...
			<textarea id="driver" name="driver" cols="120" rows="20">{:$Result.driver:}</textarea>
		</li>
		<li>
		...
		YueMi.API.Admin.invoke('runer', 'dlg_select_page', {
			...
			$.each(q.List, function (k, v) {
				html += '<li>' +
							'<div>' + v.Title + '</div><ul>';
				$.each(v.Albumn,function(k1,v1){
					html += '<li style="float:left;width:130px;height:120px;margin:5px;border:1px solid #999;text-align:center;padding-top:5px"><img src="' + v1.Picture + '" style="width:100px;" data-id="dpage-' + v.Id + '-' + v1.index + '" data-title="'+ v.Title + '"/></li>';
				});
				html += '</ul></li>';
			});
			html += '</ul>';
