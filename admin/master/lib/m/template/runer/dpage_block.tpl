...
<table border="0" cellspacing="0" cellpadding="0" class="Grid">
	<caption>
        {:$page_name:}模块管理
        <a class="button button-red" style="float:left;" href="/index.php?call=runer.dpage&parent_id={:$parent_id:}" title="返回页面">
			<i class="fas fa-angle-left"></i>
		</a>
        <a class="button button-blue" style="float:left;" href="/index.php?call=runer.dpage_block_create&page_id={:$page_id:}">
	...
	{:foreach from=$Result->Data value=v:}
		<tr>
			<td>{:$v.id:}</td>
			<td><a href="/index.php?call=runer.dpage_block_update&id={:$v.id:}&page_id={:$page_id:}">{:$v.name:}</a></td>
			<td>{:$v.alias:}</td>
			<td>{:$v.source_type:}</td>
			<td>{:$v.sizer:}</td>
			<td>{:$v.width:}</td>
			...
			<td>{:if $v.widget_id == 0:}请选择{:/if:}{:$v.widget_name:}<i class="fas fa-edit" onclick="choice_widget({:$v.id:})"></i> </td>
			<td>
				...
				<a href="/index.php?call=runer.widget_create&block_id={:$v.id:}" style="color:blue;"><i class="fas fa-edit"></i>新组件</a> |
				<a href="javascript:void(0);" onclick="javascript:do_preview({:$v.id:},{:$page_id:});" style="color:blue;"><i class="fas fa-search"></i>预览</a> 
			</td>
		</tr>
	{:/foreach:}
	<tr class="paging">
...
<form class="form_add add_content" id="widget_choice" name="form1" action="/teach.php?call=examination.examination_create_do" method="post">
	<h3 style="margin-bottom:20px;"><i class="fas fa-shield "></i>组件选择</h3>
	<label>名称:</label>
	<label>
		<select id="widget_show" style="width:200px;height:30px;background-color:#fff ">
		</select>
	</label>
	<input id="cancel" type="button" value="取 消" onclick="javascript:stop1()">
	<input class="sub" name="ok" type="button" id="buttonss" value="提 交" onclick="javascript:save_widget()">
</form>

<script type="text/javascript">

function do_preview(block_id,page_id){
	YueMi.API.Admin.invoke('runer', 'do_preview', {
...
function save_widget(){
	var id = $('#block_id').val();
	var widget = $('#widget_show').val();
	YueMi.API.Admin.invoke('runer', 'block_add_widget', {
		__access_token : '{:$User->token:}',
		block_id : id,
		widget_id: widget
	}, 
...
function choice_widget(id){
	tankuang();
	$('#widget_choice').toggle();
	load_widget();
	$('#block_id').val(id)
}
function tankuang() {
...
function load_widget(){
	YueMi.API.Admin.invoke('runer', 'widget_load', {
	__access_token : '{:$User->token:}',
		}, 
	function (t, r, q) {
		if (q.__code === 'OK')
		...
		$.confirm({
			...
			buttons: {
				accept: {
					...
					text: '删除',
					action: function () {
						YueMi.API.Admin.invoke('runer', 'dpage_block_del', {
							__access_token : '{:$User->token:}',
							id: id
						}, function (t, r, q) {
							if (q.__code === 'OK')
